...
# conoce el resultado de TODAS las VMs a la vez).
#
...
#   ##HOST_SUMMARY## <host> <PASS_total> <FAIL_total> <OK|FALLOS>
cd "$(dirname "$0")" || exit 2
source ./lib.sh
...
for t in "${tests[@]}"; do
  ...
  printf '%s\n' "$out"
  [[ $ec -ne 0 ]] && rc=1
  # Extrae "N PASS" y "M FAIL" de la línea "--- test_x en host: N PASS, M FAIL" de tests/lib.sh
...
echo
if [[ $rc -eq 0 ]]; then
  echo "RESULTADO del host $HOST: OK ($HOST_PASS PASS, $HOST_FAIL FAIL)"
  echo "##HOST_SUMMARY## $HOST $HOST_PASS $HOST_FAIL OK"
else
  echo "RESULTADO del host $HOST: HAY FALLOS ($HOST_PASS PASS, $HOST_FAIL FAIL)"
  echo "##HOST_SUMMARY## $HOST $HOST_PASS $HOST_FAIL FALLOS"
fi
exit $rc
