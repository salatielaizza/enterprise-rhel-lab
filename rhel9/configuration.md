# RHEL 9.8 — Configuración base tras la instalación

## 1. Objetivo
Dejar `rhel9-app01` registrado, actualizado y coherente con el resto del laboratorio antes de las Etapas 2-4.

## 2. Prerrequisitos
Instalación completada (`rhel9/installation.md`), acceso `ssh rhel9-app01`.
...
| Paso | Comando | Notas |
|---|---|---|
| Registro | `scripts/lab.sh register rhel9-app01 <usuario>` | La contraseña se pide en la VM, no en la línea de comandos |
| Estado | `sudo subscription-manager status` | Repos: BaseOS + AppStream tras registrar (o repo local desde la ISO). |
...
| Red | `nmcli -f NAME,FILENAME con show` | Formato de perfil: Perfiles **keyfile** por defecto ✔ (`/etc/NetworkManager/system-connections/`) |
| Inventario | `scripts/lab.sh facts rhel9-app01` | Alimenta la matriz de comparación |
...
## 8. Diferencias
Ver `rhel9/differences.md` y `comparison/matrix.md`.
